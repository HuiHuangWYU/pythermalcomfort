"""Shared helpers for review-only plotting prototypes."""
...
def _extract_output_payload(result: Any, output: str) -> Any:
    """Extract a scalar or vector-capable output payload for contour backends."""
    output_name = output.strip()
    if not output_name:
        raise ValueError("output must be a non-empty string.")
...
def _resolve_effective_columns(
    ...
    *,
    ...
    effective = (
        set(columns)
        if not allowed_args
        else {col for col in columns if col in allowed_args}
    )
